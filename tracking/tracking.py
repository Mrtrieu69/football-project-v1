...
class Tracker:
    ...
    def detect_frames(self, frames):
        ...
        #     Results(
        #         boxes=Boxes(
        #             # Mỗi box chứa thông tin: [x1, y1, x2, y2, confidence, class_id]
        #             xyxy=tensor([[100, 200, 300, 400, 0.95, 0],  # person 1
        #                         [150, 250, 350, 450, 0.87, 0],  # person 2
        #                         ...]),
        #             conf=tensor([0.95, 0.87, ...]),  # độ tin cậy
        #             cls=tensor([0, 0, ...])  # class id (0 = person)
        #         ),
        #         names={0: 'person', 1: 'ball', ...},  # mapping class_id -> tên
        #         orig_img=array(...),  # ảnh gốc
        #         path='...'  # đường dẫn ảnh (nếu có)
        #     ),
        ...
        #         keypoints=Keypoints(
        #             data=tensor([
        #                 # người 1: [x,y,conf] cho mỗi keypoint
        #                 [[120, 210, 0.9],  # điểm 1
        #                 [125, 230, 0.85], # điểm 2
        #                 [130, 250, 0.88], # điểm 3
        #                 ...],
        ...
        #                 ...]
        #             ]),
        #             conf=tensor([0.9, 0.87]),  # độ tin cậy cho mỗi người
        #             cls=tensor([0, 0])  # class id (0=person)
        #         ),
        #         names={0: 'person'},
        #         orig_img=array(...)
    ...
    def get_object_tracks(self, frames):
        ...
        for frame_number, (detection, detection_keypoints) in enumerate(zip(detections, detections_keypoints)):
            ...
            # detection_sv = Detections(
            #     xyxy=np.array([  # Tọa độ bounding boxes dạng [x1,y1,x2,y2]
            #         [100, 200, 300, 400],  # box của player 1
            #         [150, 250, 350, 450],  # box của player 2
            #         [400, 300, 420, 320],  # box của ball
            #     ]),
            #     confidence=np.array([  # Độ tin cậy của mỗi detection
            #         0.95,  # player 1
            #         0.87,  # player 2
            #         0.92   # ball
            #     ]),
            #     class_id=np.array([  # ID của class
            #         0,    # player (0)
            #         0,    # player (0)
            #         1     # ball (1)
            #     ])
            ...
            # detection_tracks = [
            #     # Mỗi phần tử có dạng: [bbox, confidence, class_id, track_id]
            #     [
            #         [150, 220, 200, 320],  # bbox
            #         0.89,                  # confidence
            #         0,                     # class_id (ví dụ: 0 = player)
            #         1                      # track_id (ID duy nhất để theo dõi đối tượng)
            #     ],
            #     [
            #         [250, 180, 300, 280],  # bbox của player khác
            #         0.92,                  # confidence
            #         0,                     # class_id
            #         2                      # track_id khác
            #     ],
